async function ka27_main_out(p1, p2, p3, imgsrc, imgalt, imgtitle) {
    let container = document.getElementById("ka-27-main")
    let main = document.createElement('div')
    main.className = 'ka-27-main'
    let mainHTML = `<p class="p-1">${p1}</p>
                        <p class="p-2">${p2}</p>
                        <p class="p-3">${p3}</p>
                        <img class="img" src="./assets/images/${imgsrc}" alt="${imgalt}" title="${imgtitle}">
                        <div class="buttons-1">
                            <button onclick="button_ka27_history()" class="button-1 button1">
                                <p class="p">Опис і історія</p>
                            </button>
                            <button onclick="button_ka27_lth()" class="button-1 button2">
                                <p class="p">ЛТХ</p>
                            </button>
                               <button onclick="button_ka27_ozbr()" class="button-1 button3">
...
async function Get_ka27_Main() {
    fetch("http://localhost:8300/Ka27MainServlet").then(
        (response) => {
        ...
        ka27_main_out(p1, p2, p3, imgsrc, imgalt, imgtitle)
    })
}
